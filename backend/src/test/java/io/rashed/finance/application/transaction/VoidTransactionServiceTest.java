...
class VoidTransactionServiceTest {
    ...
    private VoidTransactionService service;

    @BeforeEach
    void setUp() {

        repository = mock(TransactionRepository.class);
        service = new VoidTransactionService(repository);
    ...
    @Test
    void execute_voidsAPostedTransaction() {
    ...
    @Test
    void execute_rejectsReversedTransaction() {

        Transaction reversed = anExpense().reverse();
        when(repository.findById(reversed.getId())).thenReturn(Optional.of(reversed));

        assertThrows(IllegalStateException.class, () -> service.execute(reversed.getId()));
    }
